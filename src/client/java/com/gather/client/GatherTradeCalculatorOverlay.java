...
public final class GatherTradeCalculatorOverlay {
    ...
    private static final int MAX_WANTED_AMOUNT = 9999999;
    ...
    public static void register() {
        ScreenEvents.AFTER_INIT.register((client, screen, w, h) -> {
            ...
            amountField = new TextFieldWidget(client.textRenderer, 0, 0, 52, 14, Text.literal("Amount"));
            ...
            Screens.getButtons(screen).add(new ClickableWidget(0, 0, BUTTON_SIZE, BUTTON_SIZE, Text.empty()) {
                @Override
            ...
            ScreenMouseEvents.allowMouseClick(screen).register((s, click) -> {
                ...
                if (amountField != null && amountField.isVisible() && inside(mx, my,
                    ...
                    amountField.mouseClicked((Click) click, false);
    ...
    private static void renderOverlay(MerchantScreen screen, DrawContext ctx, int mx, int my, float delta) {
        if (!GatherSettings.get().enabled) return;
        updateAnimation();
        ...
        if (offers.isEmpty()) {
            ctx.drawText(client.textRenderer, Text.literal("No trades available"),
        ...
        for (int row = 0; row < rowsVisible && row + scroll < offers.size(); row++) {
            ...
            renderTradeRow(ctx, offer, panelX + PAD + 4, rowInnerW - 8, rowY + 4, offer.isDisabled());
            if (hovered) {
                ctx.setCursor(StandardCursors.POINTING_HAND);
                hoveredLines = List.of(Text.literal("Select trade output"));
                tooltipX = mx;
                tooltipY = my;
            }
        }
    ...
    private static void renderTradeRow(DrawContext ctx, TradeOffer offer, int x, int maxW, int y, boolean disabled) {
        MinecraftClient client = MinecraftClient.getInstance();
        ...
        cx += 24;
        if (!second.isEmpty()) {
            ctx.drawText(client.textRenderer, Text.literal("+"), cx, y + 5, muted, false);
            cx += 10;
        ...
        cx += 20;
        String name = sell.getName().getString();
        int nameMaxW = x + maxW - cx - 2;
        if (client.textRenderer.getWidth(name) > nameMaxW) {
            name = client.textRenderer.trimToWidth(name, nameMaxW - client.textRenderer.getWidth("...")) + "...";
        }
        ctx.drawText(client.textRenderer, Text.literal(name), cx, y + 5, text, false);
    }

    private static void renderDetails(MerchantScreen screen, DrawContext ctx, int mx, int my, int panelX, int detailTop) {
        MinecraftClient client = MinecraftClient.getInstance();
        TradeOfferList offers = offers(screen);
        if (selectedTrade < 0 || selectedTrade >= offers.size()) {
            syncAmountField(false, 0, 0);
            ctx.drawText(client.textRenderer, Text.literal("Select a trade, then enter wanted output."),
                    panelX + PAD, detailTop + 10, GatherTheme.textSecondary(), false);
            return;
        }

        TradeOffer offer = offers.get(selectedTrade);
        ItemStack sell = offer.getSellItem();
        int fieldX = panelX + PAD + client.textRenderer.getWidth("Want") + 4;
        int fieldY = detailTop + 2;
        syncAmountField(true, fieldX, fieldY);
        amountField.render(ctx, mx, my, 0.0F);
        ctx.drawText(client.textRenderer, Text.literal("Want"), panelX + PAD, detailTop + 5,
                GatherTheme.textSecondary(), false);
        ctx.drawText(client.textRenderer, sell.getName(), fieldX + 58, detailTop + 5,
        ...
        ctx.drawText(client.textRenderer,
                Text.literal("Trades needed: " + calc.trades()),
        ...
        GatherTheme.drawNineSlice(ctx,
                ...
                btnX, btnY, btnW, btnH);
        ctx.drawText(client.textRenderer, Text.literal("Add Goals"),
                btnX + 9, btnY + 4, canAdd ? GatherTheme.textButton() : GatherTheme.textDisabled(), false);
        if (hovered) {
            ctx.setCursor(StandardCursors.POINTING_HAND);
            hoveredLines = List.of(Text.literal("Add required buy items to Gather"));
            tooltipX = mx;
            tooltipY = my;
        }
        ...
        if (statusTicks > 0 && !statusText.isEmpty()) {
            ...
            int msgW = client.textRenderer.getWidth(statusText) + 8;
            ...
            ctx.drawText(client.textRenderer, Text.literal(statusText),
    ...
    private static void handleClick(MerchantScreen screen, int mx, int my, int button) {
        ...
        if (inside(mx, my, panelX + PAD, rowsTop, PANEL_W - PAD * 2, rowsVisible * ROW_H)) {
            ...
            if (offerIndex >= 0 && offerIndex < offers.size()) {
                ...
                amountField.setText(String.valueOf(Math.max(1, offers.get(offerIndex).getSellItem().getCount())));
    ...
    private static void addGoals(TradeOffer offer) {
        int wanted = wantedAmount();
        Calculation calc = calculate(offer, wanted);
        ...
        for (Cost cost : calc.costs()) {
            String itemId = Registries.ITEM.getId(cost.stack().getItem()).toString();
    ...
    private static Calculation calculate(TradeOffer offer, int wanted) {
        ItemStack sell = offer.getSellItem();
    ...
    private static int wantedAmount() {
        ...
        try {
            return Math.min(MAX_WANTED_AMOUNT, Integer.parseInt(amountField.getText().trim()));
    ...
    private static TradeOfferList offers(MerchantScreen screen) {
    ...
    private static void renderToggleButton(DrawContext ctx, int mx, int my, int x, int y) {
        ...
        if (hovered) {
            ctx.setCursor(StandardCursors.POINTING_HAND);
            hoveredLines = List.of(Text.literal(panelOpen ? "Close trade calculator" : "Open trade calculator"));
            tooltipX = mx;
            tooltipY = my;
        }
    }
    ...
    private static void renderStack(DrawContext ctx, ItemStack stack, int x, int y, boolean overlayCount) {
        if (stack.isEmpty()) return;
        GatherTheme.draw(ctx, GatherTheme.CRAFT_ITEM_SLOT, x, y);
        ctx.drawItem(stack, x + 1, y + 1);
        if (overlayCount && stack.getCount() > 1) {
            ctx.drawStackOverlay(MinecraftClient.getInstance().textRenderer, stack, x + 1, y + 1);
        }
    }

    private static void renderCostStack(DrawContext ctx, ItemStack stack, int count, int x, int y) {
        if (stack.isEmpty()) return;
    ...
    private static void renderTradeRowBackground(DrawContext ctx, int x, int y, int w, int h, boolean active) {
        GatherTheme.drawNineSlice(ctx, active ? GatherTheme.TRADE_ROW_ACTIVE : GatherTheme.TRADE_ROW, x, y, w, h);
    }

    private static void renderTradeButtonIcon(DrawContext ctx, int x, int y, boolean hovered, boolean active) {
