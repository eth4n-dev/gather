...
public class ServerRecipeBreakdown {
    ...
    public static List<BreakdownEntry> breakdown(String itemId, int count, int depth, World world) {
        List<BreakdownEntry> result = new ArrayList<>();
        recurse(itemId, count, depth, result, new HashSet<>(), world);
        return result;
    }

    public static boolean isInventoryCraftable(String itemId, World world) {
        ServerRecipeManager rm = (ServerRecipeManager) world.getRecipeManager();
        RecipeEntry<?> recipe = findRecipe(itemId, rm, world);
        if (recipe == null) return false;
        Recipe<?> r = recipe.value();
        if (r instanceof ShapedRecipe shaped) {
            return shaped.getWidth() <= 2 && shaped.getHeight() <= 2;
        }
        return r.getIngredientPlacement().getIngredients().size() <= 4;
    }

    private static void recurse(String itemId, int count, int depth,
                                 List<BreakdownEntry> result, Set<String> visiting, World world) {
    ...
    private static RecipeEntry<?> findRecipe(String itemId, ServerRecipeManager rm, World world) {
        var context = net.minecraft.recipe.display.SlotDisplayContexts.createParameters(world);
        for (RecipeEntry<?> entry : rm.values()) {
            if (entry.value() instanceof net.minecraft.recipe.SmithingRecipe) continue;
            List<net.minecraft.recipe.display.RecipeDisplay> displays = entry.value().getDisplays();
            if (displays.isEmpty()) continue;
            net.minecraft.item.ItemStack out = displays.get(0).result().getFirst(context);
     ...
     */
    private static List<String> findGatherableSourceAlternatives(String itemId, ServerRecipeManager rm,
                                                                   World world, Set<String> visiting) {
        var context = net.minecraft.recipe.display.SlotDisplayContexts.createParameters(world);
        ...
        for (RecipeEntry<?> entry : rm.values()) {
            ...
            if (!Registries.ITEM.getId(out.getItem()).toString().equals(itemId)) continue;
            ...
            if (ings.size() != 1 || !ings.get(0).isSingle()) continue; // not a simple 1-ingredient recipe
            String ingId = ings.get(0).primary();
            if (visiting.contains(ingId)) continue;
            if (!isGatherLeaf(ingId) && hasAnyRecipe(ingId, rm, world)) continue; // not a pure gatherable
            if (!sources.contains(ingId)) sources.add(ingId);
        }

        // Only return alternatives when there are genuinely multiple recipes for this item
        // and at least 2 gatherable sources were found
        if (recipesFound <= 1 || sources.size() < 2) return null;
        return sources;
    }

    private static boolean hasAnyRecipe(String itemId, ServerRecipeManager rm, World world) {
        var context = net.minecraft.recipe.display.SlotDisplayContexts.createParameters(world);
        for (RecipeEntry<?> entry : rm.values()) {
            if (entry.value() instanceof net.minecraft.recipe.SmithingRecipe) continue;
            List<net.minecraft.recipe.display.RecipeDisplay> displays = entry.value().getDisplays();
            ...
            net.minecraft.item.ItemStack out = displays.get(0).result().getFirst(context);
            if (out != null && !out.isEmpty()
                    && Registries.ITEM.getId(out.getItem()).toString().equals(itemId)) return true;
        }
        return false;
    }

    private static int getOutputCount(RecipeEntry<?> entry, World world) {
        var context = net.minecraft.recipe.display.SlotDisplayContexts.createParameters(world);
        List<net.minecraft.recipe.display.RecipeDisplay> displays = entry.value().getDisplays();
    ...
    private static List<BreakdownEntry> getIngredients(RecipeEntry<?> entry, Set<String> visiting,
                                                        ServerRecipeManager rm, World world) {
        List<BreakdownEntry> result = new ArrayList<>();
        List<Ingredient> ingredients = entry.value().getIngredientPlacement().getIngredients();
        for (Ingredient ing : ingredients) {
            List<String> allIds = ing.getMatchingItems()
                    .map(re -> Registries.ITEM.getId(re.value()).toString())
                    .distinct()
    ...
    private static boolean isReversibleStorageRecipe(String itemId, RecipeEntry<?> entry,
                                                      ServerRecipeManager rm, World world) {
        ...
        RecipeEntry<?> reverse = findDirectPackingRecipe(onlyIng.primary(), itemId, outputCount, rm, world);
        return reverse != null;
    }

    private static RecipeEntry<?> findDirectPackingRecipe(String outputItemId, String requiredIngredientId,
        ...
        for (RecipeEntry<?> entry : rm.values()) {
            ...
            net.minecraft.item.ItemStack out = displays.get(0).result().getFirst(context);
            if (out == null || out.isEmpty()) continue;
            if (!Registries.ITEM.getId(out.getItem()).toString().equals(outputItemId)) continue;
